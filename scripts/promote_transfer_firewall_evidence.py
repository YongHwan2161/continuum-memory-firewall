"""Promote one exact transfer-firewall artifact into public judge evidence."""
...
def _all_receipts(raw: dict[str, Any]) -> list[dict[str, Any]]:
    return [item["provider_receipt"] for item in raw["source_calibration"]] + [
        item["provider_receipt"] for item in raw["target_attestations"]
    ] + [
        receipt
...
def build_reference(
    ...
    *,
    ...
    if SHA_PATTERN.fullmatch(head) is None:
        raise RuntimeError("transfer firewall source head is invalid")
    if run_id < 1 or attempt < 1 or artifact_id < 1:
        raise RuntimeError("transfer firewall workflow lineage is invalid")
    expected_artifact = f"continuum-transfer-firewall-{head}-{run_id}-{attempt}"
    if artifact_name != expected_artifact:
        raise RuntimeError("transfer firewall artifact name is not exact-head bound")
    if SHA256_PATTERN.fullmatch(artifact_archive_sha256) is None:
        raise RuntimeError("transfer firewall archive digest is invalid")
    if public != build_public_transfer_firewall(raw):
        raise RuntimeError("public transfer report is not the raw-report projection")
    gate = raw.get("gate", {})
    gate_checks = [value for key, value in gate.items() if key != "status"]
    if (
        gate.get("status") != "PASS"
        or not gate_checks
        or not all(value is True for value in gate_checks)
    ):
        raise RuntimeError("transfer firewall benchmark gate did not pass")
    if raw.get("real_external_provider") is not True:
        raise RuntimeError("transfer firewall is not a real provider run")
    if raw.get("provider") != "github-actions":
        raise RuntimeError("transfer firewall provider is not GitHub Actions")
    ...
    if any(methodology.get(key) != value for key, value in expected_cardinality.items()):
        raise RuntimeError("transfer firewall live cardinality is invalid")
    if methodology.get("labels_opened_by_controller_only") is not True:
        raise RuntimeError("transfer firewall label boundary is invalid")

    receipts = _all_receipts(raw)
    run_ids = [int(item["workflow_run_id"]) for item in receipts]
    artifact_ids = [int(item["artifact_id"]) for item in receipts]
    artifact_digests = [str(item["artifact_digest"]) for item in receipts]
    if not (
        len(receipts) == 84
        ...
        raise RuntimeError("transfer firewall receipts are not exact and unique")
    if any(
        ...
        raise RuntimeError("transfer firewall receipt boundary failed")

    observations = raw.get("observations", [])
    pairs = {(item.get("arm"), item.get("case_id")) for item in observations}
    source_fingerprints = {
        str(item.get("source_environment_fingerprint")) for item in observations
    }
    target_fingerprints = {
        str(item.get("target_environment_fingerprint")) for item in observations
    }
    if (
        len(observations) != 36
        or len(pairs) != 36
        or {arm for arm, _case_id in pairs}
        != {"stateless", "raw_rag", "continuum"}
        or source_fingerprints & target_fingerprints
    ):
        raise RuntimeError("transfer firewall paired population is invalid")

    arms = raw.get("arms", {})
    continuum = arms.get("continuum", {})
    raw_rag = arms.get("raw_rag", {})
    stateless = arms.get("stateless", {})
    comparison = raw.get("paired_comparisons", {})
    vs_stateless = comparison.get("continuum_vs_stateless", {}).get(
        "same_cause", {}
    )
    vs_raw = comparison.get("continuum_vs_raw_rag", {})
    if not (
        ...
        and raw_rag.get("verified_recoveries") == 6
        and raw_rag.get("near_neighbor_false_transfers") == 6
        and raw_rag.get("unsafe_patches") == 6
        and raw_rag.get("false_canonical_promotions") == 6
        and vs_stateless.get("diagnostic_probe_reduction_cases") == 6
        and vs_stateless.get("diagnostic_probe_exact_p_value") == 0.03125
        and vs_raw.get("verified_recovery_lift_percentage_points") == 50.0
        and vs_raw.get("near_neighbor_false_transfers_prevented") == 6
    ):
        raise RuntimeError("transfer firewall result does not match the live claim")

    commitment = raw.get("commitment", {})
    seal = raw.get("seal_receipt", {})
    if seal.get("write_once_condition") != "If-None-Match:*":
        raise RuntimeError("transfer firewall seal was not write-once")
    for value in (
        ...
        if SHA256_PATTERN.fullmatch(str(value or "")) is None:
            raise RuntimeError("transfer firewall commitment lineage is invalid")
    if "not arbitrary repository repair" not in str(
        public.get("claim_boundary", "")
    ).lower():
        raise RuntimeError("transfer firewall claim boundary is missing")
    return {
        "schema_version": 1,
        "head_sha": head,
        ...
        "counterfactual_pairs": methodology["counterfactual_pairs"],
        "target_cases": methodology["target_cases"],
...
def promote(
    *,
    ...
    judge["schema_version"] = 13
    judge["generated_at"] = raw["generated_at"]
    judge["claim_boundary"] = (
        "Read-only verification of a preregistered three-arm counterfactual "
        "cross-environment transfer firewall over six paired synthetic CI fault "
        "families, 84 exact GitHub Actions receipts, prior CockroachDB vector/RLS "
        "evidence, AWS receipts, an immutable release, and Devpost lineage. The "
        "transfer result proves bounded provider-attested same-cause reuse and "
        "near-neighbor rejection on this registered population; it does not prove "
        "arbitrary repository repair or open-world semantic generalization."
    )
    judge["transfer_firewall"] = reference
    if release_tag is not None:
        ...
        release["transfer_firewall_asset_name"] = "transfer-firewall-v1.json"
        asset_name_fields = {
