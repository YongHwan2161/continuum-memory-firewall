...
| AWS identity | GitHub OIDC assumes a one-hour project deployer role whose immutable repository/environment subject and explicit deny boundaries are tested; the `continuum-production` environment admits only `main` | bootstrap once, secure the AWS root user, then log out Root and use only the dedicated role |
