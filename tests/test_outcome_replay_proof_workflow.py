...
class OutcomeReplayProofWorkflowTests(unittest.TestCase):
    ...
    def test_public_judge_surface_binds_chain_and_immutable_asset(self):
        ...
        self.assertIn("continuum-release-envelope-v2.json", page)
        self.assertIn("judge-offline-capsule-v1.json", page)
        self.assertIn("release-transaction-receipt.json", page)
        self.assertIn("github_api_requests:0", page)
        self.assertIn("same_origin_static_gets:5", page)
        self.assertIn("outcome_replay_cas_closure===true", page)
