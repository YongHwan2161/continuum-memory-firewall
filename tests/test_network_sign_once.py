...
class NetworkSignOnceTests(unittest.TestCase):
    ...
    def test_release_workflow_owns_the_only_author_signing_path(self) -> None:
        ...
        self.assertIn("Wait for candidate Pages artifact availability", pages_workflow)
        self.assertIn(
            "actions/runs/$GITHUB_RUN_ID/artifacts",
            pages_workflow,
        )
        self.assertIn(
            "browser-verified Pages artifact did not become API-visible",
            pages_workflow,
        )
        self.assertIn("steps.transaction-artifact.outputs.artifact-digest", release_workflow)
        self.assertIn("Verify the materialized candidate receipt", pages_workflow)
        self.assertIn("--to-state BROWSER_VERIFIED", pages_workflow)
        self.assertIn("Re-run the one-click judge against the final deployment", pages_workflow)
